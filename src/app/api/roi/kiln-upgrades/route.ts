...
import { SOLUTIONS, type SolutionVariant } from "@/features/kiln-upgrades-roi/solutions";
...
function buildHtml(data: {
  ...
  // ── run calcSolution for all packages ──
  type SolutionRow = {
  ...
  const productionSection = `
    ...
    <table style="border-collapse:collapse;width:100%;margin-bottom:12px;">
      ...
      ${row("Timel&oslash;nssats", e(fmtEur.format(eurPerHour) + " / time"))}
      ...
      ${row("Samlede cyklusser / uge", e(totalUnitsPerWeek.toLocaleString("da-DK")))}
    </table>
    <table style="border-collapse:collapse;width:100%;">
      <thead>
        <tr>
          <th style="${CSS.th}">Ovn / Kiln</th>
          <th style="${CSS.th}">Størrelse</th>
          <th style="${CSS.thR}">Cyklusser / uge</th>
        </tr>
      </thead>
  ...
  const calcRows = allRows.map((r) => {
    ...
    const cycleRows = data.products.map((p) => {
      ...
      return `<tr>
        ...
        <td style="font-size:12px;color:#666;padding:2px 8px;text-align:right;">${p.unitsPerWeek.toLocaleString("da-DK")} cyk/uge</td>
    ...
    return `
      ...
      <tr style="${rowBg}">
        <td style="padding:4px 10px 4px 10px;font-size:12px;color:#5a7c9a;vertical-align:top;">Ovntimer / uge</td>
      ...
      <tr style="${rowBg}">
        <td style="padding:4px 10px 4px 10px;font-size:12px;color:#5a7c9a;vertical-align:top;">Cyklustider per ovn</td>
        <td style="padding:2px 0;">
          <table style="border-collapse:collapse;width:100%;">
            <tr>
              <th style="font-size:11px;color:#aaa;padding:2px 8px 2px 16px;text-align:left;">Ovn</th>
              ...
              <th style="font-size:11px;color:#aaa;padding:2px 0;text-align:right;">Ovn-t/uge</th>
            </tr>
            ${cycleRows}
          </table>
        </td>
      ...
      <tr style="${rowBg}">
        ...
        <td style="padding:4px 0;font-size:13px;color:#0f1115;">${e(fmtEur.format(m.annualFutureCost))} <span style="color:#7a8a9a;font-size:11px;">(${fmtNum(m.weeklyMachineHours)} ovnt/uge × ${m.effectiveOperators} op. × ${WORKING_WEEKS} uger × ${fmtEur.format(eurPerHour)})</span></td>
      </tr>
      <tr style="${rowBg}">
  ...
  const calcSection = `
    ${sectionHeading("Beregning — alle opgraderingspakker")}
  ...
  if (data.selectedSolution && selectedSol) {
    selectionSection = `
      ${sectionHeading("Kundens valgte opgradering")}
      <table style="border-collapse:collapse;width:100%;">
        ${row("Pakke", "<strong>" + e(selectedSol.name) + "</strong>")}
    ...
    selectionSection = `
      ${sectionHeading("Kundens valgte opgradering")}
      <p style="font-size:13px;color:#7a8a9a;">(ingen pakke valgt)</p>`;
  }
...
<title>Kiln Upgrades ROI &#8212; salgsrapport</title>
</head>
<body style="${CSS.body}">
  <div style="${CSS.card}">
    <h1 style="${CSS.h1}">Kiln Upgrades ROI</h1>
...
export async function GET() {
  const html = buildHtml({
    contact: { name: "Lars Andersen", email: "lars@savværk.dk", job: "Driftschef", company: "Skovs Saværk A/S" },
    products: [{ id: "Lumber Kiln — Medium", name: "", size: "60 m³ chamber", unitsPerWeek: 8 }],
    operatorHoursPerWeek: 60,
    availableShifts: 2,
    country: "DK",
    selectedSolution: { name: "Heat Recovery System", automationOptions: [] },
...
export async function POST(request: Request) {
  ...
  const countryInfo = COUNTRIES[data.country] ?? { name: data.country, eurPerHour: 0 };
  const totalUnitsPerWeek = data.products.reduce((s, p) => s + p.unitsPerWeek, 0);
  const selName = data.selectedSolution?.name ?? "(ingen pakke valgt)";
...
<div style="background:#fff;border-radius:8px;padding:28px 32px;max-width:560px;margin:0 auto;color:#1a1d22;">
  ...
  <table style="border-collapse:collapse;width:100%;margin-bottom:20px;">
    ...
    <tr><td style="color:#5a7c9a;font-size:12px;padding:4px 12px 4px 0;">Cyklusser / uge</td><td style="font-size:14px;padding:4px 0;">${totalUnitsPerWeek.toLocaleString("da-DK")}</td></tr>
    <tr><td style="color:#5a7c9a;font-size:12px;padding:4px 12px 4px 0;">Valgt pakke</td><td style="font-size:14px;font-weight:600;padding:4px 0;">${e(selName)}</td></tr>
  </table>
  ...
  const bodyText = [
    ...
    `Cyklusser / uge: ${totalUnitsPerWeek.toLocaleString("da-DK")}`,
    `Valgt pakke:     ${selName}`,
    ``,
    `Den detaljerede salgsrapport med beregninger er vedhæftet.`,
  ].join("\n");

  const attachmentFilename = "kiln-upgrades-roi-" + data.contact.name.toLowerCase().replace(/[^a-z0-9]+/g, "-") + ".html";
